...
#from sound_graphics import GraphWin, Point
...
from typing import Any, cast, Callable, Dict, Iterable, Optional, Sequence
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title: str = "Graphics Window", width: int = 200, 
        ...
        master:tk.Toplevel = tk.Toplevel(_root)
        ...
        self.foreground: str = "black"
        self.items: List[GraphicsObject] = []
        self.mouseX: Optional[int] = None
        self.mouseY: Optional[int] = None
        self.bind("<Button-1>", self._onClick)
        self.bind_all("<Key>", self._onKey)
        self.height: int = int(height)
        self.width: int = int(width)
        self.autoflush: bool = autoflush
        self._mouseCallback: Optional[Callable[..., None]] = None
        self.trans: Optional[Transform] = None
    ...
    def __repr__(self) -> str:
        if self.isClosed():
            ...
            return "GraphWin('{}', {}, {})".format(self.master.title(),  # type: ignore
                                             self.getWidth(),
                                             self.getHeight())
    ...
    def __checkOpen(self) -> None:
        if self.closed:
            raise GraphicsError("window is closed")

    def _onKey(self, evnt: Event) -> None:
        self.lastKey = evnt.keysym # type: ignore
    ...
    def getMouse(self): # type: () -> Point
        ...
        assert self.mouseX is not None and self.mouseY is not None
        x,y = self.toWorld(self.mouseX, self.mouseY)
        self.mouseX = None
        self.mouseY = None
        return Point(x,y)

    def checkMouse(self): # type: () -> Optional[Point]
    ...
    def toScreen(self, x: float, y: float) -> Tuple[int, int]:
        trans = self.trans
        if trans:
            return self.trans.screen(x,y) # type: ignore
        else:
            return int(x),int(y)
                      
    def toWorld(self, x: int, y: int) -> Tuple[float, float]:
        trans = self.trans
        if trans:
            return self.trans.world(x,y) # type: ignore
        else:
            return x,y
        
    def setMouseHandler(self, func: Callable[..., None]) -> None:
        self._mouseCallback = func
        
    def _onClick(self, e: Event) -> None:
        self.mouseX = e.x # type: ignore
        self.mouseY = e.y # type: ignore
        if self._mouseCallback:
            self._mouseCallback(Point(e.x, e.y)) # type: ignore

    def addItem(self, item): # type: (GraphicsObject) -> None
        self.items.append(item)

    def delItem(self, item): # type: (GraphicsObject) -> None
...
class Transform:
    ...
    def __init__(self, w: float, h: float, xlow: float, ylow: float, 
                xhigh: float, yhigh: float) -> None:
        # w, h are width and height of window
        # (xlow,ylow) coordinates of lower-left [raw (0,h-1)]
        # (xhigh,yhigh) coordinates of upper-right [raw (w-1,0)]
        xspan: float = (xhigh-xlow)
        yspan: float = (yhigh-ylow)
        self.xbase: float = xlow
        self.ybase: float = yhigh
        self.xscale: float = xspan/float(w-1)
        self.yscale: float = yspan/float(h-1)
...
class GraphicsObject:
    ...
    # A subclass of GraphicsObject should override the _draw,
    #   _move, and clone methods.
    ...
    def __init__(self, options: List[str]) -> None:
        ...
        #    drawn shape.
        self.canvas:Optional[GraphWin] = None
        # Really this is whatever you get back from the canvas.draw_*
    ...
    def setFill(self, color: object) -> None:
        """Set interior color to color"""
        self._reconfig("fill", color)
        
    def setOutline(self, color: object) -> None:
    ...
    def draw(self, graphwin): # type: (GraphWin) -> GraphicsObject
    ...
    def _reconfig(self, option: str, setting: object) -> None:
    ...
    def _draw(self, canvas:GraphWin, options:Dict[str,object]) -> Optional[int]:
    ...
    def clone(self): # type: () -> Optional[GraphicsObject]
...
class Point(GraphicsObject):
    def __init__(self, x: float, y: float) -> None:
        ...
        self.setFill = self.setOutline # type: ignore
    ...
    def _draw(self, canvas: GraphWin, options: Dict[str, object]) -> int:
        x,y = canvas.toScreen(self.x,self.y)
        return canvas.create_rectangle(x,y,x+1,y+1,options) # type: ignore
        
    def _move(self, dx: float, dy: float) -> None:
        self.x = self.x + dx
        self.y = self.y + dy
        
    def clone(self): # type: () -> Point
...
class _BBox(GraphicsObject):
    ...
    def __init__(self, p1: Point, p2: Point, options: List[str] = ["outline","width","fill"]) -> None:
...
class Rectangle(_BBox):
    ...
    def _draw(self, canvas: GraphWin, options: Dict[str, object]) -> int:
        ...
        return canvas.create_rectangle(x1,y1,x2,y2,options) # type: ignore
...
class Oval(_BBox):
    ...
    def _draw(self, canvas: GraphWin, options: Dict[str, object]) -> int:
        ...
        x1,y1 = canvas.toScreen(p1.x,p1.y)
        x2,y2 = canvas.toScreen(p2.x,p2.y)
        return canvas.create_oval(x1,y1,x2,y2,options) # type: ignore
...
class Line(_BBox):
    ...
    def __init__(self, p1: Point, p2: Point) -> None:
        ...
        self.setOutline = self.setFill # type: ignore

    def __repr__(self) -> str:
        return "Line({}, {})".format(str(self.p1), str(self.p2))

    def clone(self) -> GraphicsObject:
        other:Line = Line(self.p1, self.p2)
        other.config = self.config.copy()
        return other
    ...
    def _draw(self, canvas: GraphWin, options: Dict[str, object]) -> int:
        ...
        x1,y1 = canvas.toScreen(p1.x,p1.y)
        x2,y2 = canvas.toScreen(p2.x,p2.y)
        return canvas.create_line(x1,y1,x2,y2,options) # type: ignore
...
class Polygon(GraphicsObject):
    ...
    def __init__(self, *points:Any) -> None:
        ...
        if len(points) == 1 and type(points[0]) == type([]):
            points = points[0]
        self.points:List[Point] = list(map(Point.clone, points))
    ...
    def _draw(self, canvas: GraphWin, options: Dict[str, object]) -> int:
        args:List[object] = [canvas]
        for p in self.points:
            x,y = canvas.toScreen(p.x,p.y)
            args.append(x)
            args.append(y)
        args.append(options)
        return GraphWin.create_polygon(*args) # type: ignore
...
class Text(GraphicsObject):
    ...
    def _draw(self, canvas: GraphWin, options: Dict[str, object]) -> int:
        p = self.anchor
        x,y = canvas.toScreen(p.x,p.y)
        return canvas.create_text(x,y,options) # type: ignore
        
    def _move(self, dx:float, dy:float) -> None:
        self.anchor.move(dx,dy)
        
    def clone(self) -> GraphicsObject:
        other = Text(self.anchor, cast(str, self.config['text']))
        other.config = self.config.copy()
        return other
    ...
    def getText(self) -> str:
        return cast(str, self.config["text"])
    ...
    def setFace(self, face:str) -> None:
        if face in ['helvetica','arial','courier','times roman']:
            f,s,b = cast(Tuple[str,int,str], self.config['font'])
    ...
    def setTextColor(self, color: object) -> None:
        self.setFill(color)
...
class Entry(GraphicsObject):
    ...
    def __init__(self, p:Point, width:int) -> None:
        ...
        self.anchor:Point = p.clone()
        #print self.anchor
        self.width:int = width
        self.text:tk.StringVar = tk.StringVar(_root)
        self.text.set("")
        self.fill:object = "gray"
        self.color:object = "black"
        self.font:Tuple[str,int,str] = cast(Tuple[str,int,str], 
                                            DEFAULT_CONFIG['font'])
    ...
    def _draw(self, canvas:GraphWin, options: Dict[str, object]) -> int:
        p:Point = self.anchor
        x,y = canvas.toScreen(p.x,p.y) # type: Tuple[float, float]
        frm = tk.Frame(canvas.master) # type: ignore
        ...
        return canvas.create_window(x,y,window=frm) # type: ignore

    def getText(self) -> str:
        return cast(str, self.text.get())
    ...
    def clone(self) -> GraphicsObject:
        other:Entry = Entry(self.anchor, self.width)
        other.config = self.config.copy()
    ...
    def _setFontComponent(self, which:int, value:Union[int, str]) -> None:
        font = list(self.font)
        font[which] = value
        self.font = cast(Tuple[str,int,str], tuple(font))
        if self.entry:
...
class Image(GraphicsObject):
    ...
    idCount:int = 0
    imageCache:Dict[int,object] = {} # tk photoimages go here to avoid GC while drawn 
    
    def __init__(self, p:Point, *pixmap:Union[str, int]) -> None:
        GraphicsObject.__init__(self, [])
        self.anchor:Point = p.clone()
    ...
    def _draw(self, canvas:GraphWin, options: Dict[str, object]) -> int:
        ...
        return canvas.create_image(x,y,image=self.img) # type: ignore
    ...
    def getWidth(self) -> int:
        ...
        return cast(int, self.img.width())

    def getHeight(self) -> int:
        """Returns the height of the image in pixels"""
        return cast(int, self.img.height())
